...
const { stripNulls, validateRegister, validateLogin, validateProfile } = require("../utils/validate");
...
router.post("/register", asyncHandler(async (req, res) => {
  ...
  if (!saved) return res.status(500).json({ error: "Failed to save registration to database." });
...
router.post("/login", asyncHandler(async (req, res) => {
  ...
  if (!user) {
    ...
    const saved = await saveDb(db);
    if (!saved) return res.status(500).json({ error: "Failed to save user profile to database." });
  }
...
router.post("/profile", asyncHandler(async (req, res) => {
  ...
  const saved = await saveDb(db);
  if (!saved) return res.status(500).json({ error: "Failed to save profile to database." });
...
router.post("/admin/login", asyncHandler(async (req, res) => {
  ...
  if (password === "sajolbd" || password === "abubd") {
    ...
    // Best-effort: only a telemetry log is at stake — never lock admins out
    // of the dashboard because a log line failed to persist.
    const saved = await saveDb(db);
    if (!saved) console.warn("admin/login: could not persist login log (continuing).");
